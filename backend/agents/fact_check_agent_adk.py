...
# CRITICAL FIX: Override system environment variable with .env file key
# This ensures we use the key from .env, not system-level environment
os.environ['GOOGLE_API_KEY'] = GEMINI_API_KEY
os.environ['GEMINI_API_KEY'] = GEMINI_API_KEY  # Also set GEMINI_API_KEY explicitly

# Validate API key is loaded correctly
if not GEMINI_API_KEY:
    raise ValueError("GEMINI_API_KEY is not set! Please check your .env file.")

# Log first 10 chars for debugging (safely)
api_key_preview = GEMINI_API_KEY[:10] + "..." if len(GEMINI_API_KEY) > 10 else "INVALID"
logger.warning("🔑 Using API key: %s (length: %d)", api_key_preview, len(GEMINI_API_KEY))

# Helper function to get a fresh client with the correct API key
def get_client():
    """Get a fresh genai client with the correct API key from .env file"""
    # Re-import to ensure we have the latest API key
    from config import GEMINI_API_KEY
    # Force override environment variables
    os.environ['GOOGLE_API_KEY'] = GEMINI_API_KEY
    os.environ['GEMINI_API_KEY'] = GEMINI_API_KEY
    # Create new client with explicit API key
    return genai.Client(api_key=GEMINI_API_KEY)

# Create initial client (will be recreated if needed)
client = get_client()
...
Return: Cleaned text ready for claim extraction"""
    ...
    try:
        # Use fresh client to ensure correct API key
        ...
        error_str = str(e)
        # If API key error, try recreating client
        if "API key" in error_str or "expired" in error_str.lower() or "400" in error_str:
            logger.warning("⚠️  API key error detected, recreating client...")
            try:
                current_client = get_client()
                response = current_client.models.generate_content(
                    model=ADK_MODEL_NAME,
                ...
                result = response.text if hasattr(response, 'text') else str(response)
                logger.warning("✅ Ingestion complete (after client recreation)")
                return result
            except Exception as e2:
                logger.warning("❌ Ingestion error (after retry): %s", str(e2)[:100])
        else:
            logger.warning("❌ Ingestion error: %s", error_str[:100])
        return input_text
    ...
    try:
        ...
        if "API key" in error_str or "expired" in error_str.lower() or "400" in error_str:
            ...
            try:
                ...
                claim = response.text.strip() if hasattr(response, 'text') else str(response)
                logger.warning("✅ Claim extraction complete (after client recreation): %s", claim[:80])
                return claim
            except Exception as e2:
                logger.warning("❌ Claim extraction error (after retry): %s", str(e2)[:100])
        else:
            logger.warning("❌ Claim extraction error: %s", error_str[:100])
